...
ADMIN_ID = int(ADMIN_ID_RAW) if ADMIN_ID_RAW else OWNER_ID
# ===== END ENV =====

YOUR_USERNAME = '@Wlzbi'
UPDATE_CHANNEL = 'https://t.me/+w6_Kos9AuPo4N2I1'
...
# === SAFE BOT REPAIR HELPERS ===
FILE_CALLBACK_PREFIX = "fc"
FILE_CALLBACK_TOKEN_LEN = 20

def _file_callback_token(script_owner_id, file_name):
    raw = f"{int(script_owner_id)}\0{file_name}".encode("utf-8")
    return hashlib.sha256(raw).hexdigest()[:FILE_CALLBACK_TOKEN_LEN]

def make_file_callback(action, script_owner_id, file_name):
    return f"{FILE_CALLBACK_PREFIX}:{action}:{int(script_owner_id)}:{_file_callback_token(script_owner_id, file_name)}"

def parse_file_callback(data):
    parts = str(data).split(":", 3)
    if len(parts) != 4 or parts[0] != FILE_CALLBACK_PREFIX:
        raise ValueError("Invalid file callback format.")
    _, action, owner_text, token = parts
    if not owner_text.isdigit():
        raise ValueError("Invalid script owner ID.")
    script_owner_id = int(owner_text)
    for entry in user_files.get(script_owner_id, []):
        if len(entry) >= 2 and _file_callback_token(script_owner_id, entry[0]) == token:
            return action, script_owner_id, entry[0]
    raise ValueError("File callback token is unknown or the file was removed.")

def get_script_key(script_owner_id, file_name):
    return (int(script_owner_id), str(file_name))

def get_script_log_path(user_folder, file_name):
    digest = hashlib.sha256(str(file_name).encode("utf-8", errors="ignore")).hexdigest()[:24]
    return os.path.join(user_folder, f".script_{digest}.log")


def is_bot_running(script_owner_id, file_name):
    script_key = get_script_key(script_owner_id, file_name)
    with BOT_SCRIPTS_LOCK:
        ...
        if script_info and script_info.get('process'):
            try:
                ...
                if not is_running:
                    logger.warning(f"Process {script_info['process'].pid} for {script_key} found but not running/zombie. Cleaning up.")
...
def install_missing_dependencies(script_path, user_folder, message_obj_for_reply):
    try:
        ...
        for module in imports:
            if module in sys.builtin_module_names:
                continue
            if module in ['os', 'sys', 're', 'time', 'datetime', 'json', 'logging', 'threading', 'subprocess', 'zipfile', 'tempfile', 'shutil', 'sqlite3', 'atexit', 'signal', 'ast', 'importlib']:
                continue
            try:
                ...
                try:
                    ...
                    logger.error(f"Failed to install {module}: {e.stderr}")
                    failed.append(module)
        if installed:
            bot.reply_to(message_obj_for_reply, f"✅ Installed missing dependencies: {', '.join(installed)}")
        if failed:
            bot.reply_to(message_obj_for_reply, f"❌ Failed to install: {', '.join(failed)}. Check logs.")
        return True
    except Exception as e:
        logger.error(f"Error installing dependencies for {script_path}: {e}", exc_info=True)
        bot.reply_to(message_obj_for_reply, f"❌ Error during dependency installation: {str(e)}")
        return False
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        if attempt == 1:
            ...
            try:
                result = subprocess.run(check_command, cwd=user_folder,
                                        stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                        text=True, encoding="utf-8", errors="ignore", timeout=30)
                if result.returncode != 0:
                    error_summary = (result.stderr or result.stdout or "Python syntax check failed.")[-3500:]
                    bot.reply_to(message_obj_for_reply, f"❌ Python syntax error in '{file_name}':\n{error_summary}")
                    return
            except subprocess.TimeoutExpired:
                bot.reply_to(message_obj_for_reply, f"❌ Python syntax check timed out for '{file_name}'.")
                return
            except Exception as e:
                logger.error("Python syntax check failed: %s", e, exc_info=True)
                bot.reply_to(message_obj_for_reply, f"❌ Could not validate '{file_name}': {e}")
                return
        logger.info(f"Starting long-running Python process for {script_key}")
        log_file_path = os.path.join(user_folder, get_script_log_path(user_folder, file_name))
        log_file = None
        process = None
        try:
        ...
        try:
            ...
            bot.reply_to(message_obj_for_reply, f"✅ Python script '{file_name}' started! (PID: {process.pid}) (User: {script_owner_id})")
...
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        if attempt == 1:
            ...
            try:
                result = subprocess.run(check_command, cwd=user_folder,
                                        ...
                                        text=True, encoding="utf-8", errors="ignore", timeout=30)
                if result.returncode != 0:
                    error_summary = (result.stderr or result.stdout or "JavaScript syntax check failed.")[-3500:]
                    bot.reply_to(message_obj_for_reply, f"❌ JavaScript syntax error in '{file_name}':\n{error_summary}")
                    return
            except FileNotFoundError:
                bot.reply_to(message_obj_for_reply, "❌ 'node' was not found. Install Node.js to run JS bots.")
                return
            except subprocess.TimeoutExpired:
                bot.reply_to(message_obj_for_reply, f"❌ JavaScript syntax check timed out for '{file_name}'.")
                return
            except Exception as e:
                logger.error("JavaScript syntax check failed: %s", e, exc_info=True)
                bot.reply_to(message_obj_for_reply, f"❌ Could not validate '{file_name}': {e}")
                return
        ...
        try:
            ...
            bot.reply_to(message_obj_for_reply, f"✅ JS script '{file_name}' started! (PID: {process.pid}) (User: {script_owner_id})")
        except FileNotFoundError:
...
# ===== NEW: Helper functions for dependency installation =====
...
def install_dependencies_for_script(script_path, user_folder, file_name, chat_id, message_obj):
    req_path = os.path.join(user_folder, 'requirements.txt')
    installed = []
    failed = []
    try:
        bot.send_message(chat_id, f"📦 Installing dependencies for `{file_name}`…")
        if os.path.exists(req_path):
            bot.send_message(chat_id, f"📄 Found `requirements.txt` – installing…")
            try:
                ...
                bot.send_message(chat_id, f"❌ Error installing requirements.txt:\n```\n{e.stderr}\n```")
        else:
            imports = get_imports_from_script(script_path)
            if not imports:
                bot.send_message(chat_id, "ℹ️ No import statements found or could not parse the script.")
                return
            for module in imports:
                if module in sys.builtin_module_names:
                    continue
                try:
                    ...
                    try:
                        ...
                        failed.append(module)
        if installed:
            bot.send_message(chat_id, f"✅ Installed: {', '.join(installed)}")
        if failed:
            bot.send_message(chat_id, f"❌ Failed to install: {', '.join(failed)}. Check logs.")
        if not installed and not failed:
            bot.send_message(chat_id, f"ℹ️ No new dependencies required or all already installed.")
    except Exception as e:
        logger.error(f"Error in install_dependencies_for_script: {e}", exc_info=True)
...
def create_control_buttons(script_owner_id, file_name, is_running=True):
    ...
    if is_running:
        markup.row(
            types.InlineKeyboardButton("🛑 Stop", callback_data=make_file_callback('stop', script_owner_id, file_name)),
            types.InlineKeyboardButton("🔄 Restart", callback_data=make_file_callback('restart', script_owner_id, file_name))
        )
        markup.row(
            types.InlineKeyboardButton("🗑️ Delete", callback_data=make_file_callback('delete', script_owner_id, file_name)),
            types.InlineKeyboardButton("📜 Logs", callback_data=make_file_callback('logs', script_owner_id, file_name))
        )
    else:
        markup.row(
            types.InlineKeyboardButton("▶️ Start", callback_data=make_file_callback('start', script_owner_id, file_name)),
            types.InlineKeyboardButton("🗑️ Delete", callback_data=make_file_callback('delete', script_owner_id, file_name))
        )
        markup.row(
            types.InlineKeyboardButton("📜 View Logs", callback_data=make_file_callback('logs', script_owner_id, file_name))
        )
    markup.row(types.InlineKeyboardButton("📦 Install Deps", callback_data=make_file_callback('installdeps', script_owner_id, file_name)))
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            for member in zip_ref.infolist():
                ...
                if not real_member_path.startswith(os.path.realpath(temp_dir)):
        ...
        for root, _, names in os.walk(temp_dir):
            for name in names:
                all_extracted_files.append(os.path.join(root, name))

        py_files = [f for f in all_extracted_files if f.lower().endswith('.py')]
        js_files = [f for f in all_extracted_files if f.lower().endswith('.js')]

        def _rel_name(path):
            return os.path.relpath(path, temp_dir).replace(os.sep, '/')

        py_rel = [_rel_name(f) for f in py_files]
        js_rel = [_rel_name(f) for f in js_files]
        req_candidates = [f for f in all_extracted_files if os.path.basename(f).lower() == 'requirements.txt']
        pkg_candidates = [f for f in all_extracted_files if os.path.basename(f).lower() == 'package.json']
        req_file = _rel_name(req_candidates[0]) if req_candidates else None
        pkg_json = _rel_name(pkg_candidates[0]) if pkg_candidates else None
        if req_file:
            req_path = os.path.join(temp_dir, req_file)
            logger.info(f"requirements.txt found, installing: {req_path}")
            bot.reply_to(message, f"📦 Installing Python dependencies from `{req_file}`...")
            try:
                command = [sys.executable, '-m', 'pip', 'install', '-r', req_path]
                result = subprocess.run(command, capture_output=True, text=True, check=True, encoding='utf-8', errors='ignore')
                logger.info(f"pip install from requirements.txt OK. Output:\n{result.stdout}")
                bot.reply_to(message, f"✅ Python dependencies from `{req_file}` installed.")
            except subprocess.CalledProcessError as e:
                error_msg = f"❌ Failed to install Python dependencies from `{req_file}`.\nLog:\n```\n{e.stderr or e.stdout}\n```"
                logger.error(error_msg)
        ...
        if pkg_json:
            ...
            bot.reply_to(message, f"📦 Installing Node dependencies from `{pkg_json}`...")
            try:
                command = ['npm', 'install']
                ...
                bot.reply_to(message, f"✅ Node dependencies from `{pkg_json}` installed.")
            except FileNotFoundError:
                bot.reply_to(message, "❌ 'npm' not found. Cannot install Node dependencies.")
                return
            except subprocess.CalledProcessError as e:
                error_msg = f"❌ Failed to install Node dependencies from `{pkg_json}`.\nLog:\n```\n{e.stderr or e.stdout}\n```"
                logger.error(error_msg)
                if len(error_msg) > 4000:
                    error_msg = error_msg[:4000] + "\n... (Log truncated)"
                bot.reply_to(message, error_msg)
        ...
        if not main_script_name:
            if py_files:
                main_script_name = py_rel[0]
                file_type = 'py'
            elif js_files:
                main_script_name = js_rel[0]
                file_type = 'js'
        if not main_script_name:
        ...
        bot.reply_to(message, f"✅ Files extracted. Starting main script: `{main_script_name}`...")
        if file_type == 'py':
...
def _logic_send_welcome(message):
    ...
    if bot_locked and user_id not in admin_ids:
        bot.send_message(chat_id, "🔒 Bot is currently locked by administrator. Please try again later.")
        return
    ...
    if user_id not in active_users:
        ...
        try:
            ...
            if photo_file_id:
                bot.send_photo(OWNER_ID, photo_file_id, caption=f"🖼️ Pic of new user {user_id}")
        except Exception as e:
    ...
    welcome_msg_text = (f"🌟 Welcome, {user_name}!\n\n"
                        ...
                        f"🤖 This bot allows you to host and run Python (`.py`) or JavaScript (`.js`) scripts.\n"
                        f"📤 You can upload single scripts or `.zip` archives containing your bot code.\n\n"
                        f"👇 Use the buttons below or type commands to get started.")
    main_reply_markup = create_reply_keyboard_main_menu(user_id)
    try:
        if photo_file_id:
            bot.send_photo(chat_id, photo_file_id)
        bot.send_message(chat_id, welcome_msg_text, reply_markup=main_reply_markup)
    except Exception as e:
...
def _logic_upload_file(message):
    user_id = message.from_user.id
    if bot_locked and user_id not in admin_ids:
        bot.reply_to(message, "🔒 Bot is locked. Uploads are disabled.")
        return
    file_limit = get_user_file_limit(user_id)
    current_files = get_user_file_count(user_id)
    if current_files >= file_limit:
        limit_str = str(file_limit) if file_limit != float('inf') else "Unlimited"
        bot.reply_to(message, f"⚠️ File limit ({current_files}/{limit_str}) reached. Please delete some files first.")
        return
    bot.reply_to(message, "📤 Please send your Python (`.py`), JavaScript (`.js`), or ZIP (`.zip`) file.")
...
def _logic_check_files(message):
    ...
    for file_name, file_type in sorted(user_files_list):
        ...
        markup.add(types.InlineKeyboardButton(btn_text, callback_data=make_file_callback('file', user_id, file_name)))
...
def handle_file_upload_doc(message):
    ...
    if bot_locked and user_id not in admin_ids:
        bot.reply_to(message, "🔒 Bot is locked. Cannot accept files.")
        return
    file_limit = get_user_file_limit(user_id)
    current_files = get_user_file_count(user_id)
    if current_files >= file_limit:
    ...
    try:
        try:
            ...
            bot.send_message(OWNER_ID, f"⬆️ Uploaded file '{file_name}' from {message.from_user.first_name} (`{user_id}`)")
        except Exception as e:
...
def install_deps_callback(call):
    try:
        _, script_owner_id, file_name = parse_file_callback(call.data)
        ...
        if file_type != 'py':
            bot.answer_callback_query(call.id, "📦 Dependency installation is only supported for Python (.py) files.", show_alert=True)
            return

        bot.answer_callback_query(call.id, "📦 Installing dependencies…")
        ...
        threading.Thread(target=install_dependencies_for_script,
                         args=(script_path, user_folder, file_name, chat_id, call.message)).start()

        try:
            bot.edit_message_text(f"⏳ Installing dependencies for `{file_name}`… This may take a moment.",
                                  chat_id, call.message.message_id)
        except Exception as e:
            logger.error(f"Error updating message for install deps: {e}")

    except (ValueError, IndexError) as e:
        logger.error(f"Error parsing installdeps callback: {e}")
        bot.answer_callback_query(call.id, "❌ Invalid data.", show_alert=True)
    except Exception as e:
        logger.error(f"Error in install_deps_callback: {e}", exc_info=True)
        bot.answer_callback_query(call.id, "❌ Error starting installation.", show_alert=True)
...
def handle_callbacks(call):
    ...
    if bot_locked and user_id not in admin_ids and data not in ['back_to_main', 'speed', 'stats']:
        bot.answer_callback_query(call.id, "🔒 Bot is locked by administrator.", show_alert=True)
        return
    try:
        if data == 'upload':
            ...
            check_files_callback(call)
        elif data.startswith('fc:file:'):
            file_control_callback(call)
        elif data.startswith('fc:start:'):
            start_bot_callback(call)
        elif data.startswith('fc:stop:'):
            stop_bot_callback(call)
        elif data.startswith('fc:restart:'):
            restart_bot_callback(call)
        elif data.startswith('fc:delete:'):
            delete_bot_callback(call)
        elif data.startswith('fc:logs:'):
            logs_bot_callback(call)
        elif data.startswith('fc:installdeps:'):
            install_deps_callback(call)
        elif data == 'speed':
            speed_callback(call)
...
def upload_callback(call):
    ...
    bot.send_message(call.message.chat.id, "📤 Please send your Python (`.py`), JavaScript (`.js`), or ZIP (`.zip`) file.")
...
def start_bot_callback(call):
    try:
        ...
        if not os.path.exists(file_path):
            bot.answer_callback_query(call.id, f"⚠️ Error: File `{file_name}` missing! Please re-upload.", show_alert=True)
            remove_user_file_db(script_owner_id, file_name)
            check_files_callback(call)
            return
        ...
        try:
            _, script_owner_id_err, file_name_err = parse_file_callback(call.data)
...
def delete_bot_callback(call):
    try:
        ...
        user_folder = get_user_folder(script_owner_id)
        file_path = os.path.join(user_folder, file_name)
        log_path = os.path.join(user_folder, get_script_log_path(user_folder, file_name))
        deleted_disk = []
        if os.path.exists(file_path):
            try:
                os.remove(file_path)
        ...
        try:
            ...
            bot.send_message(chat_id_for_reply, f"🗑️ Record `{file_name}` deleted.")
    except (ValueError, IndexError) as e:
...
def process_broadcast_message(message):
    ...
    bot.reply_to(message, f"⚠️ Confirm Broadcast:\n\n```\n{preview_text}\n```\n"
                          f"To **{target_count}** users. Sure?", reply_markup=markup)
...
def execute_broadcast(broadcast_text, photo_id, video_id, caption, admin_chat_id):
    ...
    for i, user_id_bc in enumerate(users_to_broadcast):
        try:
            if broadcast_text:
                bot.send_message(user_id_bc, broadcast_text)
            elif photo_id:
                bot.send_photo(user_id_bc, photo_id, caption=caption if caption else None)
            elif video_id:
                bot.send_video(user_id_bc, video_id, caption=caption if caption else None)
            sent_count += 1
        except telebot.apihelper.ApiTelegramException as e:
...
def process_add_admin_id(message):
    ...
    try:
        ...
        try:
            bot.send_message(new_admin_id, "🎉 Congratulations! You are now an Admin.")
        except Exception as e:
