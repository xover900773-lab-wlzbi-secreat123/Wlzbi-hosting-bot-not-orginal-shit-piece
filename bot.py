...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        if attempt == 1:
            ...
            check_proc = None
            try:
                check_proc = subprocess.Popen(check_command, cwd=user_folder, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='ignore', stdin=subprocess.DEVNULL)
                ...
                if return_code != 0 and stderr:
                    ...
                    if match_py:
                        ...
                        bot.reply_to(message_obj_for_reply, f"📦 Missing module: `{module_name}`. Attempting install...")
                        try:
                            subprocess.run([sys.executable, '-m', 'pip', 'install', module_name], check=True, capture_output=True, text=True, encoding='utf-8', errors='ignore')
                            bot.reply_to(message_obj_for_reply, f"✅ Installed `{module_name}`. Retrying...")
                            time.sleep(2)
                            threading.Thread(target=run_script, args=(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt + 1)).start()
                            return
                        except subprocess.CalledProcessError as e:
                            bot.reply_to(message_obj_for_reply, f"❌ Failed to install `{module_name}`: {e.stderr}")
                            return
                    else:
                        install_missing_dependencies(script_path, user_folder, message_obj_for_reply)
                        error_summary = stderr[:500]
                        bot.reply_to(message_obj_for_reply, f"⚠️ Error in script pre-check for '{file_name}':\n```\n{error_summary}\n```\nFix the script or dependencies.", parse_mode='Markdown')
                        return
            except subprocess.TimeoutExpired:
                logger.info("Python Pre-check timed out (>5s), imports likely OK. Proceeding to long run.")
    ...
    try:
        ...
        if attempt == 1:
            ...
            try:
                ...
                if return_code != 0 and stderr:
                    ...
                    if match_js:
                        ...
                        if not module_name.startswith('.') and not module_name.startswith('/'):
                            ...
                            bot.reply_to(message_obj_for_reply, f"📦 Missing Node module: `{module_name}`. Attempting install...")
                            try:
                                subprocess.run(['npm', 'install', module_name], cwd=user_folder, check=True, capture_output=True, text=True, encoding='utf-8', errors='ignore')
                                bot.reply_to(message_obj_for_reply, f"✅ Installed Node module `{module_name}`. Retrying...")
                                time.sleep(2)
                                threading.Thread(target=run_js_script, args=(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt + 1)).start()
                                return
                            except subprocess.CalledProcessError as e:
                                bot.reply_to(message_obj_for_reply, f"❌ Failed to install Node module `{module_name}`: {e.stderr}")
                                return
                        else:
                ...
                logger.info("JS Pre-check timed out (>5s), imports likely OK. Proceeding to long run.")
                if check_proc and check_proc.poll() is None:
                    check_proc.kill()
                    check_proc.communicate()
            except FileNotFoundError:
...
def create_control_buttons(script_owner_id, file_name, is_running=True):
    ...
    markup.row(types.InlineKeyboardButton("📦 Install Deps", callback_data=f'installdeps_{script_owner_id}_{file_name}'))
...
def handle_file_upload_doc(message):
    ...
    try:
        try:
            ...
            bot.send_message(OWNER_ID, f"⬆️ Uploaded file '{file_name}' from {message.from_user.first_name} (`{user_id}`)", parse_mode='Markdown')
        except Exception as e:
...
def install_deps_callback(call):
    try:
        _, script_owner_id_str, file_name = call.data.split("|", 2)
...
def handle_callbacks(call):
    ...
    try:
        ...
        elif data.startswith('installdeps|'):
...
def file_control_callback(call):
    try:
        _, script_owner_id_str, file_name = call.data.rsplit('_', 2)
        script_owner_id = int(script_owner_id_str)
        requesting_user_id = call.from_user.id
        if not (requesting_user_id == script_owner_id or requesting_user_id in admin_ids):
...
def start_bot_callback(call):
    try:
        ...
        try:
            _, script_owner_id_err_str, file_name_err = call.data.rsplit('_', 2)
